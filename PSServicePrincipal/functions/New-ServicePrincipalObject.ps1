Function New-ServicePrincipalObject
{
        ...
        .EXAMPLE
            PS c:\> New-ServicePrincipalObject -CreateSPNWithName -ServicePrincipalName NameOfSPNToBeCreated

            This example creates a new Service Princpial with name "NameOfSPNToBeCreated" and password (an autogenerated GUID) and creates the service principal based on the application just created. The start date and end date are added to password credential.

        .EXAMPLE
            PS c:\> New-ServicePrincipalObject -CreateBatchSPNS -NameFile c:\temp\Namefile.txt -BatchJob
        ...
        .EXAMPLE
            ...
            This example creates a new Active Directory Service Principal for the application with application id '34a28ad2-dec4-4a41-bc3b-d22ddf90000e'.

        .EXAMPLE
            PS c:\> New-ServicePrincipalObject -CreateSPNsWithNameAndCert -ServicePrincipalName NameOfSPNToBeCreated -Certificate <public certificate as base64-encoded string>

            This example creates a new application with name "NameOfSPNToBeCreated" and certifcate and creates the service principal based on the application just created. The end date is added to key credential.
