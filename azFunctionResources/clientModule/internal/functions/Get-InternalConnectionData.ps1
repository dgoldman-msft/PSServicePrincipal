...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'PSServicePrincipal.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'PSServicePrincipal.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'PSServicePrincipal.Client.ProtectedToken'
