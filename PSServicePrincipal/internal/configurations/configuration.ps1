...
$script:CertFolder = Join-Path ([Environment]::GetFolderPath("MyDocuments")) "\Self-Signed Certificates"
if(-NOT (Test-Path -Path $script:CertFolder)) { New-Item -Path $script:CertFolder -ItemType Directory}

Set-PSFConfig -Module 'PSServicePrincipal' -Name 'Cert.CertFolder' -Value "$($script:CertFolder)" -Initialize -Validation string -Handler { } -Description "The backup path where to save self-signed certificates. Supports some placeholders such as %Date% to allow for timestamp in the name. For full documentation on the supported wildcards, see the documentation on https://psframework.org"
