...
$parentModule = 'PSServicePrincipal'
if (-not $ModuleName) { $ModuleName = 'PSServicePrincipal.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\PSServicePrincipal'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\PSServicePrincipal.psd1"
$prereqHash = @{
